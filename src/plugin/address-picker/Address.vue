...
//筛选出各省级对象
var pObj = Address.filter(findcity)
//获取各省级对象名字
var provinces = pObj.map(getAddressName)
export default {
  name: 'address',
  props: {
  ...
  watch: {
    showAddressPicker: function (old, val) {
      this.show = !val
    },
    init: function (val) {
      this.initVal = val;
  ...
  methods: {
    ...
    initAddress: function () { //初始化地址组件
      ...
      vm.slots[4].values.map(function (val, index) {
